...
'>
  {currentUser?.user ? (
    ...
    <LuUser className="w-6 h-6 text-[var(--text)]" />
  )}
</MenuButton>
      ...
      >
        <MenuItems className="absolute right-0 top-full mt-2 w-56 origin-top-right rounded-md bg-[var(--background-primary)] shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-10">
          <div className="px-2 py-2 rounded-md">
            <MenuItem>
              {({ active }) => (
                <button
                  onClick={() => navigate('/settings')}
                  className={`${active ? 'bg-[var(--background-secondary)] text-[var(--accent)]' : 'text-[var(--text)]'
